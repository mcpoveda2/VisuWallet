...
export function obtenerResumenFinanciero(
  ...
  const ingresosTotal = calcularIngresos(todasLasTransacciones.length > 0 ? todasLasTransacciones : transaccionesActuales);
  const gastosTotal = calcularGastos(todasLasTransacciones.length > 0 ? todasLasTransacciones : transaccionesActuales);
